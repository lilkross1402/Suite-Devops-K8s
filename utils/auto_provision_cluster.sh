...
auto_provision_ha_cluster() {
    ...
    for node in "${all_nodes[@]}"; do
        ...
        if ! ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no "${ssh_user}@${node}" "echo connected" &>/dev/null; then
            log_error "No se pudo conectar vía SSH a ${ssh_user}@${node}. Asegúrese de tener la clave SSH autorizada."
            return 1
        fi
    done
    log_success "Conectividad SSH verificada exitosamente en todos los nodos."
    ...
    for node in "${master_ips[@]}"; do
        ...
        ssh -o StrictHostKeyChecking=no "${ssh_user}@${node}" "cd /home/${ssh_user}/kubeops-suite 2>/dev/null || cd /root/kubeops-suite 2>/dev/null || true; git fetch origin && git reset --hard origin/main && chmod +x kubeops.sh modules/*.sh stack/*.sh lib/*.sh utils/*.sh && sudo ./stack/deploy_haproxy_keepalived.sh" || true
    done
    ...
    ssh -o StrictHostKeyChecking=no "${ssh_user}@${master1_ip}" "sudo kubeadm init --control-plane-endpoint ${vip_ip}:8443 --upload-certs --skip-phases=addon/kube-proxy || true"
    ...
    local cert_key
    cert_key=$(ssh -o StrictHostKeyChecking=no "${ssh_user}@${master1_ip}" "sudo kubeadm init phase upload-certs --upload-certs 2>/dev/null | tail -n 1")
    local join_token
    join_token=$(ssh -o StrictHostKeyChecking=no "${ssh_user}@${master1_ip}" "sudo kubeadm token create --print-join-command 2>/dev/null")
    ...
    for ((i=1; i<${#master_ips[@]}; i++)); do
        ...
        ssh -o StrictHostKeyChecking=no "${ssh_user}@${node}" "sudo ${join_token} --control-plane --certificate-key ${cert_key} || true"
    done
    ...
    for node in "${worker_ips[@]}"; do
        ...
        ssh -o StrictHostKeyChecking=no "${ssh_user}@${node}" "sudo ${join_token} || true"
    done
    ...
    ssh -o StrictHostKeyChecking=no "${ssh_user}@${master1_ip}" "sudo kubectl get nodes -o wide"
    printf "\n"
}
